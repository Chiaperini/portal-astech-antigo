...
if($row_busca = mysqli_fetch_assoc($result_busca)){
	...
	$sql_cidade_assistente = "SELECT nome FROM cidade WHERE cod_ibge = '{$row_busca['cod_cidade']}'";
	...
	if ($mysqli->query($sql_i) === TRUE) { //inserido no banco com sucesso
		...
		{
			$sql_pecas = "INSERT into pecas_solicitadas_vendas ( id_usuario, numero_fica, id_peca, cod_equipamento, cod_peca, descricao, quantidade, data ) VALUES ('{$_SESSION['id_usuario']}', '{$numero_ficha}', '{$id_peca}', '{$_POST['codEquipamento'][$i]}', '{$_POST['codPeca'][$i]}', '{$_POST['descricao'][$i]}', '{$_POST['quantidade'][$i]}', '{$data}')";
		...
		<strong>Cidade do Assistente</strong>&nbsp;'.$cidade_assistente.'<br></font>
		<br>
		...
		$mail->Host = "mail.chiaperini.com.br"; // Endereço do servidor SMTP
		$mail->SMTPAuth = true; // Usa autenticação SMTP? (opcional)
		$mail->Port = 587; //  Usar 587 porta SMTP
		$mail->Username = 'chiaperini@chiaperini.com.br'; // Usuário do servidor SMTP
		$mail->Password = 'C@chi2018'; // Senha do servidor SMTP
		...
		// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
		$email = 'amanda@chiaperini.com.br'; //mudar depois para astecnica
		...
		<strong>Estado do Assistente</strong>&nbsp;'.$estado_assistente.'<br></font>';
		...
		if ($enviado) {
			...
			$mail_destinatario->Host = "mail.chiaperini.com.br"; 
			$mail_destinatario->SMTPAuth = true; 
			$mail_destinatario->Port = 587; 
			$mail_destinatario->Username = 'chiaperini@chiaperini.com.br'; 
			$mail_destinatario->Password = 'C@chi2018'; 
