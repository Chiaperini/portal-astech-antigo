					...
					if(isset($_FILES['InputFotosProduto'])){
						...
						$upload_ok = 0;	
					     for($i = 0; $i < count($tmp_name); $i++){ //passa por todos os arquivos
							...
							$tmpext = explode('.', $name[$i]);
							$ext = '.'.strtolower(array_values(array_slice($tmpext, -1))[0]);
							// echo "<center>";
							// echo $ext;
							// print_r($tmpext);
							// print_r($allowedExts);
							// echo "</center>";
						 	...
						 	if(in_array($ext, $allowedExts)){ //Pergunta se a extensão do arquivo, está presente no array das extensões permitidas
								...
								$new_name2 = date("Y.m.d-H.i.s") ."-". $i . $ext;
		 						$dir_foto = 'media/uploads/'.$new_name2;
		            			$image = WideImage::load($tmp_name[$i]); //Carrega a imagem utilizando a WideImage
		            			$image = $image->resize(770, 780, 'outside'); //Redimensiona a imagem para 170 de largura e 180 de altura, mantendo sua proporção no máximo possível
					            $image = $image->crop('center', 'center', 770, 780); //Corta a imagem do centro, forçando sua altura e largura
					 
					            $image->saveToFile($dir_foto); //Salva a imagem
						...
						if(($upload_ok/count($tmp_name)) == 1){
							...
							<strong>Nota Fiscal:</strong>&nbsp; https://portalastech.chiaperini.com.br/media/uploads/'.$dir_nota.'<br></font>';
