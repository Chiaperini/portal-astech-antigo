...
if($pagina == 'lojistas'){
	...
	$busca_email = "SELECT * FROM lojistas WHERE email = '{$email}'";
	$result_email = mysqli_query($mysqli, $busca_email);
	$row_email = mysqli_num_rows($result_email);
	if($row_email != '0'){
		$erro = 1;
	...
	if($erro == 0){
		$insert_lojista = "INSERT into lojistas(codigo, nome_empresa, CNPJ, email, nome_fantasia, endereco, complemento, bairro, cep, cod_cidade,
		estado, ddd, telefone, inscricao_estadual, status, data_insercao) VALUES('{$codigo}','{$nome_empresa}', '{$cnpj}', '{$email}', '{$nome_fantasia}' , '{$endereco}' ,'{$complemento}',
		'{$bairro}', '{$cep}', '{$cidade}', '{$estado}', '{$ddd}', '{$telefone}', '{$inscricao_estadual}', '1', '{$date}')  ";
		if ($mysqli->query($insert_lojista) === TRUE){
				
			$insert_log = "INSERT into log_informacoes(id_user, nome, email, acao, local, data) VALUES('{$_SESSION['id_admin']}', '{$_SESSION['nome_admin']}', '{$_SESSION['usuario_admin']}', 'insercao' , 'Lojista {$nome_empresa}{$id_lojista}' ,'{$_SESSION['ultimoAcesso_admin']}')  ";
	    	$mysqli->query($insert_log);
	    	
			?><script>alert('Lojista inserido com sucesso');window.location.href = "http://portalastech.chiaperini.com.br/admin/pages/lojistas.php?acao=gerenciar";</script><?php
		}
		else{
			?><script>alert('Erro ao atualizar assistente. Verifique sua conexão ou entre em contato com o setor responsável');window.history.go(-1);</script><?php
		}
