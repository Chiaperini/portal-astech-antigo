<?php
	...
	/* ---------- LOJISTAS ---------- */
	if($pagina == 'lojistas'){
		$id_lojista = $_POST['input_id_lojista'];
		$codigo = $_POST['input_codigo_lojista'];
		$nome_empresa = utf8_encode($_POST['input_nome_empresa_lojista']);
		$cnpj = preg_replace("/\D+/", "", $_POST['input_cnpj_lojista']);
		$email = $_POST['input_email_lojista'];
		$nome_fantasia = utf8_encode($_POST['input_nome_fantasia_lojista']);
		$endereco = utf8_encode($_POST['input_endereco_lojista']);
		$complemento = utf8_encode($_POST['input_complemento_lojista']);
		$bairro = utf8_encode($_POST['input_bairro_lojista']);
		$cidade = $_POST['select_cidade_lojista'];
		$estado = $_POST['select_estado_lojista'];
		$cep = $_POST['input_cep_lojista'];
		$ddd = $_POST['input_ddd_lojista'];
		$telefone = $_POST['input_telefone_lojista'];
		$inscricao_estadual = $_POST['input_inscricao_estadual_lojista'];
		
		$sql_update = "UPDATE lojistas SET codigo = '{$codigo}', nome_empresa ='{$nome_empresa}', CNPJ = '{$cnpj}', email = '{$email}', nome_fantasia = '{$nome_fantasia}', 
		endereco = '{$endereco}', complemento = '{$complemento}', bairro = '{$bairro}', cep = '{$cep}', cod_cidade = '{$cidade}', estado = '{$estado}',
		ddd = '{$ddd}', telefone = '{$telefone}', inscricao_estadual = '{$inscricao_estadual}' WHERE id='{$id_lojista}'";
    	if($result_update = mysqli_query($mysqli, $sql_update)){
    		
			$local = utf8_encode($nome_empresa);
			$insert_admin = "INSERT into log_informacoes(id_user, nome, email, acao, local, data) VALUES('{$_SESSION['id_admin']}', '{$_SESSION['nome_admin']}', '{$_SESSION['usuario_admin']}', 'atualizacao' , 'Lojista {$local}{$id_lojista}' ,'{$date_log}')  ";
    		if ($mysqli->query($insert_admin) === TRUE){
    			?><script>alert('Lojista atualizado com sucesso');window.location.href = "http://portalastech.chiaperini.com.br/admin/pages/lojistas.php?acao=gerenciar";</script><?php
    		}
    	}
		else{
			?><script>alert('Erro ao atualizar lojista. Verifique sua conexão ou entre em contato com o setor responsável');window.history.go(-1);</script><?php
		}
	}
	/* ------ USUÁRIOS ADMIN ------- */
